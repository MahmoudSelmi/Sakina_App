...
class _ReciterDetailScreenState extends State<ReciterDetailScreen> {
  ...
  Future<void> _downloadAllSurahs(BuildContext context) async {
    ...
    if (!allowed) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'التحميل مفعّل على الواي فاي بس دلوقتي - وصّل واي فاي وجرّب تاني')),
        );
      }
      return;
    }
    ...
    if (items.isEmpty) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('كل السور محمّلة عندك بالفعل')));
      }
      return;
    }
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: ValueListenableBuilder<int>(
          ...
          builder: (context, done, _) =>
              Text('اتحمّل $done من ${items.length}'),
        ),
        actions: [
          TextButton(
            onPressed: () {
    ...
    if (!cancelled && context.mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('اتحمّلت ${items.length} سورة بنجاح')));
    }
  }
